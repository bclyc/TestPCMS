...
public ArrayList<Integer> combineColumns = new ArrayList<>();/**用于保存需要合并的列号**/
...
public CombineData(String[][] datas, int... combineColumns) {
	this.datas = datas;
	for (int i = 0; i < combineColumns.length; i++) {/**初始化合并列号*/
...
**/
    for (int i = 0; i < datas.length; i++) {
    	String[] data = datas[i];
/**检查datas[i]的第integer列是否等于pre( 前一列),*/    
    if (pre.equals(data[integer])) {/** 相当于*datas[i][integer-]=datas[i][integer]*/
           count++; }  
    else { /**不等于*/
       rowCount.put(start, count);
...
public int span(int row, int column) {
	  int index = combineColumns.lastIndexOf(column);/** 返回列表中最后出现指定元素的索引，如果列表不包含此元素，则返回 -1。**/
	  if (index != -1) {
